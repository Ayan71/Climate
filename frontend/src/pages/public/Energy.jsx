...
import { Zap, Eye, Search, Filter } from 'lucide-react';
...
const Energy = () => {
  ...
  const filtered = energyList.filter(d => {
    ...
    <div className="space-y-6 pb-12 font-sans">
      ...
      <div className="p-6 rounded bg-white border border-slate-200 shadow-sm space-y-2">
        <div className="flex items-center space-x-2 text-xs font-bold text-amber-800 uppercase tracking-wider">
          ...
          <span>Domain Portal: Energy (/energy)</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900">Renewable & Primary Energy Statistics</h1>
        <p className="text-slate-600 text-xs max-w-2xl leading-relaxed">
          National and state-level statistics on installed solar capacity, wind generation, biomass, and energy transition metrics across India.
        </p>
      </div>
      ...
      <div className="bg-white p-3.5 rounded border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs shadow-sm">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Search Energy datasets..."
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white rounded border border-slate-200 text-slate-500">
          ...
          <p className="font-bold text-sm text-slate-800">No Energy datasets match the current filter.</p>
        </div>
      ) : (
