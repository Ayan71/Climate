...
const Power = () => {
  ...
  const filtered = powerList.filter(d => {
    ...
    <div className="space-y-6 pb-12 font-sans">
      ...
      <div className="p-6 rounded bg-white border border-slate-200 shadow-sm space-y-2">
        <div className="flex items-center space-x-2 text-xs font-bold text-blue-800 uppercase tracking-wider">
          ...
          <span>Domain Portal: Power (/power)</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900">National Grid & Power Infrastructure</h1>
        <p className="text-slate-600 text-xs max-w-2xl leading-relaxed">
          High-voltage substation locations, peak load demand trends, and regional electricity consumption figures published by national grid authorities.
        </p>
      </div>
      ...
      <div className="bg-white p-3.5 rounded border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs shadow-sm">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Search Power datasets..."
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white rounded border border-slate-200 text-slate-500">
          ...
          <p className="font-bold text-sm text-slate-800">No Power datasets match the current filter.</p>
        </div>
      ) : (
